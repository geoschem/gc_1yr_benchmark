;to plot annual zonal mean 222Rn,210Pb and 7Be concentrations
;hyl,1/19/2012 (hongyu.liu-1@nasa.gov)

pro rnpbbe_annual_zonalmean, Year, Met, Res, Version, InDir, InFile, OutDir, $
                             PS=PS, _EXTRA=e

   ; Output filename
   OutFile = OutDir + Version + '.zonal_mean_annual.ps'
...
;------------------------------------------------------------------------------
         ...
         for L = 0, lev2-1 do begin  
            std_zm (J,L, IMN) = total ( std_3d(*,J,L) ) / n_elements(xmid)
         endfor
         endfor
      ...
      contour, annual_zm, ymid(*), pmid(0:lev2-1),levels=levs,          $
      ...
      fig_title =  'GEOS-Chem ' + Version + ' Annual Zonal Mean, ' + year

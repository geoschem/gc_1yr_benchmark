...
;        bmy, 25 Mar 2008: Cleaned up and removed extraneous stuff.  Also now
;                          use drawing colors from the !MYCT variable
...
;                          intead of the default 3 versions
   ...
   filest = 'data/netCDF/'+Species+'.stations'
   ...
   scales = [1.5, 1.5, 1.5, 1.5, 1.5, 2.5, 1.5, 2.5, 1.5, 2, 2.5, $
             2, 1.5, 1.5, 1.5, 1, 1,1.5,2.5,1,1,1,1,1,1,1,1,1.5,1,$
             1.5, 2, 1.5, 1.5, 1, 1, 1, 1, 2, 2, 2, 1.5]
   ...
   for k=1, max_sta do begin
      ...
      for w=0, Hk-1 do begin
         if gtestd(w) gt 0. then begin
            ...
            ; Plot error bars
            oplot, errbar/1000, [-alog10(tmp(w)), -alog10(tmp(w))], $
                   color=!MYCT.BLACK
            nnum = strtrim(string(gtenum(w), format='(i4)'), 2)
            xyouts, howfarover, -alog10(tmp(w)), $
                    nnum, charsize=0.6, /data, color=!MYCT.BLACK
         endif
      endfor
      ...
      ; Plot profile
      oplot, co/2, -alog10(pressure), $
             psym=-sym(1), symsize=0.2, linestyle=3, color=!MYCT.BLUE
      endif
...
;if ncount eq nrow*ncol then begin
      xyouts, 0.05, 0.5, 'Pressure (hPa)', /normal, $
              align=0.5, orientation=90, charsize=1.2, color=!MYCT.BLACK

      xyouts, 0.5, 0.05, 'C2H6 (ppb)', $

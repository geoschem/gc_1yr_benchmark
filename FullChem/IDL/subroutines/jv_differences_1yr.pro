...
;
; PURPOSE:
;        Creates difference (new - old) plots of J-values at both
...
;
; CALLING SEQUENCE:
;        JV_DIFFERENCES_1YR, FILES, VERSIONS, [, Keywords ]
;
; INPUTS:
...
;                     'v10-01h-geosfp-Run2', $
;                     'v10-01i-geosfp-Run0'  ]
;        PSNAME   = 'JValue_Differences_Jan.v10-01i-geosfp-Run0.ps'
; 
;        JV_DIFFERENCES_1YR, FILES, VERSIONS, Month='Jan', $
;                            /PS, OUTFILENAME=PSNAME
; 
;
;             ; Creates J-value difference plots of three GEOS-CHEM versions
;             ; (in this case v10-01i - v10-01e and v10-01i - v10-01h)
;             ; for Jan 2013.
;
; MODIFICATION HISTORY:
;        mps, 02 Dec 2013: - Initial version based on jv_diff.pro from
;                            GAMAP v2-18
...
; with subject "IDL routine JV diffs"
;-----------------------------------------------------------------------

pro PlotJVDiff, InData1,     InData2,     Version1, Version2, $
                TracerName,  GridInfo,    Unit,     Month,    $
                L_Sfc=L_Sfc, L_500=L_500, PS=PS,    _EXTRA=e
   
   ;====================================================================
   ; Internal routine PlotJVDiff plots either the surface or 500 hPa
   ; difference of tracer between old and new versions (bmy, 11/9/07)
   ;====================================================================
   ...
   if ( Keyword_Set( L_Sfc ) )                              $
      then Title = VerStr + '!C!C' + TracerName     +       $
                   ' - J-Value Diff @ surface for ' + Month

   ; Plot title for 500 hPa
   if ( Keyword_Set( L_500 ) )                              $
      then Title = VerStr + '!C!C' + TracerName     +       $
                   ' - J-Value Diff @ 500 hPa for ' + Month  
   ...
   TvMap, Diff, XMid, Ymid,                                  $
      ...
      Divisions=Divisions, /Sample,         /Grid,           $
...
pro JV_Differences_1yr, Files, Versions, Month=Month,$
   ...
   if ( N_Elements( OutFileName ) ne 1 ) then OutFileName = 'jv_diff.ps'
...
;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
   ...
   for D = 0L, N_Elements( DataInfo_1 ) - 1L do begin
      ...
      ; Debug
      ;Print, 'Creating ', Month, ' J-value diff maps for ', TracerName_1

      if ( nVersions eq 2 ) then begin

         ; Plot the surface differences -- Version 2 - Version 1
         PlotJVDiff, Data_Sfc_1,   Data_Sfc_2, Versions[0], Versions[1], $
                     TracerName_1, GridInfo_1, Unit,        Month,       $
                     /L_Sfc,       PS=PS,      _EXTRA=e

         ; Plot the 500 hPa differences -- Version 2 - Version 1
         PlotJVDiff, Data_500_1,   Data_500_2, Versions[0], Versions[1], $
                     TracerName_1, GridInfo_1, Unit,        Month,       $
                     /L_500,       PS=PS,      _EXTRA=e
         
      endif else begin
      
         ; Plot the surface differences -- Version 3 - Version 2
         PlotJVDiff, Data_Sfc_2,   Data_Sfc_3, Versions[1], Versions[2], $
                     TracerName_1, GridInfo_1, Unit,        Month,       $
                     /L_Sfc,       PS=PS,      _EXTRA=e

         ; Plot the 500 hPa differences -- Version 3 - Version 2
         PlotJVDiff, Data_500_2,   Data_500_3, Versions[1], Versions[2], $
                     TracerName_1, GridInfo_1, Unit,        Month,       $
                     /L_500,       PS=PS,      _EXTRA=e

         ; Plot the surface differences -- Version 3 - Version 1
         PlotJVDiff, Data_Sfc_1,   Data_Sfc_3, Versions[0], Versions[2], $
                     TracerName_1, GridInfo_1, Unit,        Month,       $
                     /L_Sfc,       PS=PS,      _EXTRA=e

         ; Plot the 500 hPa differences -- Version 3 - Version 1
         PlotJVDiff, Data_500_1,   Data_500_3, Versions[0], Versions[2], $
                     TracerName_1, GridInfo_1, Unit,        Month,       $
                     /L_500,       PS=PS,      _EXTRA=e

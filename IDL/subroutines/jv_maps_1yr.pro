...
;
; PURPOSE:
;        Creates maps of J-values at both the surface and 500 hPa
;        from 1-year benchmark simulation output. 
;       
; CATEGORY:
;        GEOS-Chem Benchmarking
;
; CALLING SEQUENCE:
;        JV_MAPS_1YR, FILES, VERSIONS, [, Keywords ]
;
; INPUTS:
...
;                     'v10-01h-geosfp-Run2', $
;                     'v10-01i-geosfp-Run0'  ]
;        PSNAME   = 'JValue_Maps_Jan.v10-01i-geosfp-Run0.ps'
; 
;        JV_MAPS_1YR, FILES, VERSIONS, Month='Jan', $
;                            /PS, OUTFILENAME=PSNAME
; 
;
;             ; Creates J-value maps of three GEOS-CHEM versions
;             ; (in this case v10-01e, v10-01h, and v10-01i)
;             ; for Jan 2013.
;
; MODIFICATION HISTORY:
;        mps, 02 Dec 2013: - Initial version based on jv_map.pro from
;                            GAMAP v2-18
...
; with subject "IDL routine JV maps"
;-----------------------------------------------------------------------

pro PlotJVMap, Data, Version, TracerName,  GridInfo, Unit, Month, $
   ...
   if ( Keyword_Set( L_Sfc ) )                              $
      then Title = Version + '!C!C' + TracerName     +      $
                   ' - J-Value @ surface for ' + Month

   ; Plot title for 500 hPa
   if ( Keyword_Set( L_500 ) )                              $
      then Title = Version + '!C!C' + TracerName     +      $
                   ' - J-Value @ 500 hPa for ' + Month  
   ...
   ; Plot J-values over a world map
...
pro JV_Maps_1yr, Files, Versions, Month=Month, $
...
;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
   ...
   for D = 0L, N_Elements( DataInfo_1 ) - 1L do begin
      ...
      ; Debug
      ;Print, 'Creating ', Month, ' J-value maps for ', TracerName_1

      ; Plot the surface maps -- Version 1
      PlotJVMap, Data_Sfc_1,   Versions[0], TracerName_1, GridInfo_1, $
                 Unit,         Month,       /L_Sfc,       PS=PS,      $
                 _EXTRA=e

      ; Plot the 500 hPa maps -- Version 1
      PlotJVMap, Data_500_1,   Versions[0], TracerName_1, GridInfo_1, $
                 Unit,         Month,       /L_500,       PS=PS,      $
                 _EXTRA=e

      ; Plot the surface maps -- Version 2
      PlotJVMap, Data_Sfc_2,   Versions[1], TracerName_2, GridInfo_2, $
                 Unit,         Month,       /L_Sfc,       PS=PS,      $
                 _EXTRA=e

      ; Plot the 500 hPa maps -- Version 2
      PlotJVMap, Data_500_2,   Versions[1], TracerName_2, GridInfo_2, $
                 Unit,         Month,       /L_500,       PS=PS,      $
                 _EXTRA=e

      if ( nVersions eq 3 ) then begin
      ; Plot the surface maps -- Version 3
      PlotJVMap, Data_Sfc_3,   Versions[2], TracerName_3, GridInfo_3, $
                 Unit,         Month,       /L_Sfc,       PS=PS,      $
                 _EXTRA=e

      ; Plot the 500 hPa maps -- Version 3
      PlotJVMap, Data_500_3,   Versions[2], TracerName_3, GridInfo_3, $
                 Unit,         Month,       /L_500,       PS=PS,      $
                 _EXTRA=e
      endif

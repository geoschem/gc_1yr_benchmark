...
pro Read_Ratio_Range, InputFile
   ...
   while ( not EOF( Ilun ) ) do begin
      ...
      ; Save into arrays
      Names  [N] = StrUpCase( StrTrim( Result[0], 2 ) )   ; Tracer name
      LoBound[N] = Float    (          Result[1]      )   ; Low bound
      HiBound[N] = Float    (          Result[3]      )   ; High bound
      UnitStr[N] = StrTrim  (          Result[4], 2   )   ; Unit string
...
Next:
   ...
   Names   = Names  [0:N-1L]
   LoBound = LoBound[0:N-1L]
   HiBound = HiBound[0:N-1L]
   UnitStr = UnitStr[0:N-1L]

...
;
;        DIFFERENCES_1YR, FILES, ALTRANGE, TRACERS, VERSIONS, $
;             /DYNRANGE, /PS, OUTFILENAME=PSNAME, MONTH=1
;
...
pro PlotAODDiff, Data1,      Data2,    Version1, Version2, $
   ...
   case ( StrUpCase( StrTrim( TracerName, 2 ) ) ) of 
      'OPD'   : NewTracerName = 'Dust: Column AOD diff for '
      'OPSO4' : NewTracerName = 'SO4: Column AOD diff for '
      'OPBC'  : NewTracerName = 'BC: Column AOD diff for '
      'OPOC'  : NewTracerName = 'OC: Column AOD diff for '
      'OPSSA' : NewTracerName = 'SALA: Column AOD diff for '
      'OPSSC' : NewTracerName = 'SALC: Column AOD diff for '
      ; For v10-01i and later
      'OPSO4550' : NewTracerName = 'SO4: Column AOD diff for '
      'OPBC550'  : NewTracerName = 'BC: Column AOD diff for '
      'OPOC550'  : NewTracerName = 'OC: Column AOD diff for '
      'OPSSA550' : NewTracerName = 'SALA: Column AOD diff for '
      'OPSSC550' : NewTracerName = 'SALC: Column AOD diff for '
   endcase
...
pro Differences_AOD_1yr, Files,             Tracers, Versions,                $
   ...
   for D = 0L, N_Elements( DataInfo_1 )-1L do begin
      ...
      if ( nVersions eq 2 ) then begin
         ...
         ; "Blue" - "Green"
         PlotAODDiff, Data_Col_2,   Data_Col_3, Versions[1], Versions[2], $
                      TracerName_1, GridInfo_1, DynRange,    Month,       $
                      _EXTRA = e
 
         ; "Blue" - "Red"
         PlotAODDiff, Data_Col_1,   Data_Col_3, Versions[0], Versions[2], $
                      TracerName_1, GridInfo_1, DynRange,    Month,       $
                      _EXTRA = e
